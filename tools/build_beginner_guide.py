...
def tabs(group, labels, extra=''):
    out = f'<div class="tabs {extra}" role="tablist" aria-label="{e(group)}" data-tabs>'
...
def build():
    ...
    body += [f'<a href="#{key}">{title}</a>' for key,title in [('rules','Basic rules'),('lineups','Starter lineups'),('tacticians','Tacticians'),('keywords','Keywords'),('videos','Expert guides')]]
    ...
    for i, x in enumerate(LINEUPS):
        ...
        body.append('<h4>Recommended equipment</h4>'+dl(x['equipment'])+'<h4>Recommended tacticians</h4>'+dl(x['tacticians'])+'<h4>Recommended talents</h4>'+dl(x['talents'])+'<h4>Hero card effects</h4>'+dl(CARD_EFFECTS[i])+'</details></div>')
    ...
    body.append('<section id="tacticians"><h2><span>03</span>Recommended tacticians</h2><p class="section-intro">Explore each tactician’s skill, Secret Technique, and exclusive talent.</p>'+tabs('tacticians',[e(t[0]) for t in TACTICIANS]))
    for i,(name,asset,blurb,skills) in enumerate(TACTICIANS):
        ...
        group=f'skill-{i}'
        labels=[img(f'hero/skill{i+1}_{j+1}.png','')+label for j,label in enumerate(['Skill','Secret Technique','Exclusive'])]
        ...
        body.append('</div></div>')
    body.append('</section><section id="keywords"><h2><span>04</span>Keyword dictionary</h2><p class="section-intro">Understand the effects written on your cards.</p><label class="search">Search keywords<input id="keyword-search" type="search" placeholder="Try: combat, levels, revive…"></label><p id="keyword-status" class="status" role="status">13 keywords shown</p><div class="keywords">')
    for name,text,vid in KEYWORDS:
        body.append('<article class="keyword"><h3>'+e(name)+'</h3><p>'+e(text)+f'</p><a href="https://wxq.qq.com/cp/a202609xszy/index.html" data-video="{vid}" target="_blank" rel="noopener">▶ Original demonstration · Chinese ↗</a></article>')
